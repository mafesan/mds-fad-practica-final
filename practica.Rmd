...
```{r read_file}
data_apps_all = read.csv('/Users/edgli/Documents/App_Store_Practica/AppleStore.csv')
```
...
Realacionamos nuestra variable de interes con las variables anteriormente mencionadas como objetivos secuendarios de la practica, asi determinar mas adelante su influencia en el modelo y si
...
```{r, fig.align='center', fig.show='hold', echo=FALSE, message=TRUE, warning=FALSE}
...
Puesto que tenemos los valores originales de las columnas donde hemos simulado los datos faltantes, podemos obtener una medida de cómo de aproximada ha sido la imputación. La medida que hemos elegido es el error cuadrático medio:
...
ggplot(data=data_train, aes(x=data_train$rating_count_tot))+
...
Observamos que se acerca mas hacia una distribución normal, sin embargo, al corroborarlo con el Shapiro-Wilk test no es asi
...
```{r}
shapiro.test(data_train$rating_count_tot_new)
```
...
Es importante recalcar, que de no haber eliminado al inicio los valores cuyo rating_count_tot correspondian a 0, al aplicar logaritmo se hubieran eliminado de igual manera, aun asi, lo mejor era eliminarlo y trabajar en función de los datos de interes


Para analizar mas a fondo el comportamiento de esta variable, estudiamos sus quantiles.
...
Tenemos outliers que probablemente alteraran el modelo, dada la dispersión entre estos es importante a considerar.
...
```{r, warning= FALSE, fig.align='center', fig.show='hold', fig.width=6, fig.height=4, echo=FALSE, message=FALSE}
ggplot(data_train, aes(x=log10(lang.num))) +
  geom_histogram(bins=10, fill='steelblue')+
...
#Rename
names(data_train)[names(data_train) == "data_train17+"] <- "rating17"
names(data_train)[names(data_train) == "data_train12+"] <- "rating12"
names(data_train)[names(data_train) == "data_train9+"] <- "rating9"
names(data_train)[names(data_train) == "data_train4+"] <- "rating4"
```
...
```{r include=FALSE}
disp_soport <- data.frame(data_train$X, data_train$sup_devices.num)

names(disp_soport)[names(disp_soport) == "data_train.X"] <- "X"
names(disp_soport)[names(disp_soport) == "data_train.sup_devices.num"] <- "disp_soport"
```

```{r}
unique(disp_soport$disp_soport)
```

```{r}
#Creamos las 4 categorías.

disp_soport$disp_categoria <- 0
disp_soport$disp_categoria [disp_soport$disp_soport < 20] <-"Menor_20"
disp_soport$disp_categoria [disp_soport$disp_soport >= 20 & disp_soport$disp_soport < 30] <- "20_Menor_30"
disp_soport$disp_categoria [disp_soport$disp_soport >= 30 & disp_soport$disp_soport < 40] <- "30_Menor_40"
disp_soport$disp_categoria [disp_soport$disp_soport >= 40] <- "Mayor_40"

disp_categoria <- select(disp_soport, X, disp_categoria)
unique(disp_categoria$disp_categoria)
```

```{r}
# Analizamos la nueva variable
disp_categoria <- data.frame(table(disp_categoria$disp_categoria))
disp_categoria = mutate(disp_categoria, percent = Freq/ sum(Freq))
names(disp_categoria)[names(disp_categoria) == "Var1"] <- "Categoría Num. Disp. soportados"
disp_categoria
```

```{r warning=FALSE}
# Integramos con Apple_Store y creamos una columna para cada categoría.
disp_soport <- select(disp_soport, X, disp_categoria)
data_train <- merge (x=data_train, y=disp_soport, by="X")

data_train <- cbind(data_train, dummy(data_train$disp_categoria))
names(data_train)[names(data_train) == "data_trainMenor_20"] <- "c_Menor_20"
names(data_train)[names(data_train) == "data_train20_Menor_30"] <- "c_20_Menor_30"
names(data_train)[names(data_train) == "data_train30_Menor_40"] <- "c_30_Menor_40"
names(data_train)[names(data_train) == "data_trainMayor_40"] <- "c_Mayor_40"
```
...
data_train_clean <- data_train %>%
  select(-c(track_name, ver, cont_rating, currency, prime_genre, rating_count_tot_new, rating_count_ver, X, id))
```
...
```
data_train_modelo_fwd <- subset(data_train_clean, select=c(rating_count_tot, size_bytes, price, user_rating_ver, ipadSc_urls.num,
         lang.num, size_bytes_new, lang.num_new, rating12, Entertainment_genre))
...
```
residuos_fwd <- rstandard(lm_fit)
valores_ajust <- fitted(lm_fit)
plot(valores_ajust, residuos_fwd)
```
...
sw_test <- shapiro.test(residuos_fwd)
```
