...
# Renombramos variables para evitar errores
colnames(data_train_clean)[14] <- "rating_12"
colnames(data_train_clean)[15] <- "rating_17"
colnames(data_train_clean)[16] <- "rating_4"
colnames(data_train_clean)[17] <- "rating_9"

colnames(data_train_clean)[21] <- "Lifestyle_genre"
colnames(data_train_clean)[22] <- "Utilities_genre"
...
Al observar el gráfico Q-Q, sí que parece que los errores se distribuyen de una manera bastante normal, aunque al ejecutar el test de Shapiro-Wilk obtenemos un p-valor muy bajo.
...
El estadístico de Durbin-Watson toma valor 2 cuando los residuos son completamente independientes (entre 1.5 y 2.5 se considera que
existe independencia). Un valor menor que 2 indica autocorrelación positiva y mayor que dos indica autocorrelación negativa. En nuestro caso, estamos muy cerca del 1.5, por lo que podríamos decir que existe independencia.

# Conclusiones de la práctica

El data set no presenta una cantidad de variables considerables para estimar su *Número de Descargas* (aproximado al número de valoraciones totales) a través de un modelo de Regresión Lineal, sin embargo, en base al estudio del dominio y las variables presentes en el data set podemos resaltar lo siguiente:

-	El tamaño de las aplicaciones puede alterar el éxito de una aplicación, la mayoría de las aplicaciones hoy en día se basan en la facibilidad que tiene tenerlas en el móvil.
-	El rating de las aplicaciones aunque no parece ser un determinante, sí que es cierto que el valor que mas se repite corresponde a un 4.5 .
-	El idioma, es una variable que se comporta un tanto peculiar, sin embargo es algo que se debe estudiar a fondo porque ya mas que importar la cantidad de idiomas a los que se encuentra traducido importa en que idiomas se encuentra traducido.
-	El género influye directamente, ya que la mayoría de las aplicaciones son para el disfrute y entretenimiento.

A manera de cierre, recalcar que la popularidad (aceptación y/o fama) es un término un tanto complejo, si bien es cierto existen diversos patrones que te permiten relacionar qué variables contribuyen a que suceda, pero debe conocerse el dominio en un gran nivel para plantear criterios y tomar decisiones acordes.

# Continuación de la práctica - 2º trimestre

Exportamos los dataframes a ficheros CSV, para poder seguir parte del análisis desde Python.

```{r}
# Nombramos y creamos la carpeta nueva donde exportamos los archivos
export_dir <- "exported-dataframes"
dir.create(export_dir)

# Exportamos los dataframes a ficheros CSV
write.csv(data_train, path(export_dir, "apps_data_train", ext=".csv"))
write.csv(data_train_clean, path(export_dir, "apps_data_train_clean", ext=".csv"))
write.csv(data_test, path(export_dir, "apps_data_test", ext=".csv"))
```
